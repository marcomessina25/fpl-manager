...
>
> **Current planning baseline:** V0.8 is delivered and merged on `master`. Active development is on branch `v09` delivering V0.9 ([`docs/v09/v09.md`](v09/v09.md)). Phase 0 (baseline freeze), Phase 1 (residual error dataset & 9-class error taxonomy), Phase 2 (structured hierarchical participation architecture), Phase 3 (learned regularized statistical models), Phase 4 (probability calibration via Platt scaling & Isotonic regression), and Phase 5 (dynamic rotation regimes & turnaround rest fingerprints) are implemented and verified across 243 passing tests. High-confidence false positives are reduced by 78.7% and manager decision penalty by 77.6% over frozen V0.8.
>
